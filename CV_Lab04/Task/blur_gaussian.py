...
# by setting sigma=-1, the value of sigma is computed automatically as: sigma = 0.3*((ksize-1)*0.5 - 1) + 0.8
...
# Now we create a 2D filter We use matrix multiplication to create an m by m 2D filter
# out of "m by 1" and "1 by m" 1D filters,
# which in this case happens to be the same thing as correlation between 1D filters
...
cv2.imshow('blurred_Gaussian',Jgb)
cv2.waitKey()
