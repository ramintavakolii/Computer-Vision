...
for filename in fnames:
    ...
    nC, CC, stats, centroids = cv2.connectedComponentsWithStats(C)  # C: This is the binary image where each pixel is either 0 (background) or 255 (foreground). 

    J = I.copy()
    for i in range(1, nC):
        cv2.circle(J, (int(centroids[i, 0]), int(
            centroids[i, 1])), 3, (0, 0, 255))
    cv2.imshow('corners', J)
    if cv2.waitKey(0) & 0xFF == ord('q'):
        break
    ...
    corners = cv2.cornerSubPix(G, np.float32(
